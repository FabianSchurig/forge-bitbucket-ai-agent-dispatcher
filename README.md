...
1. The panel reads the active issue key from the Forge context.
2. `getJiraContext` (resolver, `asUser`) fetches the issue **summary** and
   produces an injection-safe suggested branch name.
...
| `$JIRA_ISSUE_SUMMARY` | Summary text of the Jira issue |
...
**Security.** Issue summaries are user-controlled free-form text, so they are
*never* concatenated into shell commands or URLs:

- Branch names are generated by `src/jira/branchName.ts`, which slugifies the
  summary down to `[A-Za-z0-9._/-]` (a summary like `"; rm -rf * ;"` collapses to
  an inert slug) and validates the result against the shared branch allowlist.
...
- Reads use `asUser` so the caller's own Jira/Bitbucket permissions apply.
...
| `read:jira-work` | Read the Jira issue summary in the issue-context dispatcher panel |
